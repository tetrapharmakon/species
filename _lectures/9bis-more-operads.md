...
- ...$Com$ consists of a family of maps $E(m)\times A^m \cong A^m\to A$, i.e. an $m$-ary operation $\xi^{(m)} : A^m \to A$ for every $m\ge 0$, each of which is *completely symmetric*, meaning $\xi^{(m)}(a_1,\dots, a_m) = \xi^{(m)}(a_{\tau 1},\dots, a_{\tau m})$  for every $\tau \in S_m$. In fact, one can prove that the operations of arity $m\ge 2$ are reducible to $\xi^{(2)} : A\times A \to A$ (which together with the associativity request, equips $A$ with a commutative semigroup structure).
- ...$As$ consists of a family of maps $\xi^{(m)} : S_m\times A^m \to A$; an element $(\sigma;(x_1,\dots,x_m))$ can be thought of as a linear order on its second component: $x_{\sigma 1} < \dots < x_{\sigma m}$, in such a way that $\xi^{(m)}(\sigma;\vec x)$ prescribes the product of the $x_i$'s in the order specified by $\sigma$ (?)