...
We have seen that to a combinatorial species we can associate at least two important invariant "quantities", i.e. two formal power series from which we can deduce properties of the species. If $\|X_n\|$ denotes the cardinality of a set, and $F : {\bf B} \to {\bf Set}$ is a combinatorial species, then
...
where the notation $\pi_0 F[n]$ denotes the quotient set $F[n]/S_n$ under the action of the symmetric group $S_n$ on $F[n]$. (This is no more and no less than the space of orbits of the action $S_n \curvearrowright F[n]$.)
...
The generating series of a combinatorial species is a powerful, but relatively coarse, invariant of the species: for example, the species $S$ of permutations and the species $L$ of linear orders have the same generating series because $\|{S}[n]\| = n! = \| {L}[n] \|$, so that
...
Computations of a few simple $Z$'s; for $E$ and $\cal C$. In general, computations of $Z_F$'s that do not use sophisticated infinite-series summing techniques are rare, or even impossible. More often than not, an explicit formula for $Z_F$ relies on mysterious combinatorial identities (through which a clever manipulation of $Z_F$'s can shed light on *even more mysterious* combinatorial identities).
...
We will see in the [following lecture](./05-species-and-analytic-functors-wide-pullbacks.html) that every species $F$ can be written as the sum of its "even" part and its "odd" part, thanks to the fact that every species $F$ admits a canonical decomposition as a coproduct
...
For example, we can split the species of permutations into its even and odd part as follows:
