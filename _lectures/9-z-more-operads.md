...
The introduction of virtual species provides results for operadic structures that are not possible in the semi-ring of ordinary species. Specifically, every virtual species $G$ where $G(0) = 0$ and $G'(0) = 1$ possesses a unique substitutional inverse $G^{\langle -1 \rangle}$. This inverse satisfies the identities $G \circ G^{\langle -1 \rangle} = X$ and $G^{\langle -1 \rangle} \circ G = X$. This allows for the explicit resolution of combinatorial functional equations of the form $F = A \circ \Psi$, where the solution is determined as $\Psi = A^{\langle -1 \rangle} \circ F$. For instance, the species of rooted trees $A$ has a substitutional inverse $A^{\langle -1 \rangle} = X \cdot E^{-1}$, which is used to define the virtual species $\Lambda = X + E_2 - X^2$ in the context of the dissymmetry theorem for trees.